...
class _TraumDetailSheetState extends State<TraumDetailSheet> {
  ...
  // Section keys for programmatic control
  ...
  Widget build(BuildContext context) {
    return StreamBuilder<DocumentSnapshot<Map<String, dynamic>>>(
      ...
      builder: (ctx, snapshot) {
        ...
        // Prefill transcript controller from live data if needed
          ...
          child: SafeArea(
            child: Column(
              children: [
                              ...
                              Row(
                                ...
                                children: [
                                  IconButton(
                                    ...
                                    onPressed: () async {
                                      final result = await _openEditBottomSheet(context, data);
                                      if (result != null) {
                                  ...
                                  IconButton(
                                    ...
                                    onPressed: () async { await _reanalyze(context, data); },
                                  ),
                                ],
                              ),
                            ],
                ...
                // Sticky mini audio player (like Prophetie screen)
  ...
  Future<Map<String, dynamic>?> _openEditBottomSheet(BuildContext context, Map<String, dynamic> data) {
                          ...
                          Expanded(
                            child: ElevatedButton(
                              ...
                              onPressed: () {
                                ...
                                if (_titleController.text.trim() != (data['title'] ?? '').toString().trim()) {
                                  updateData['title'] = _titleController.text.trim();
                                }
                                if (_creatorController.text.trim() != (data['creatorName'] ?? '').toString().trim()) {
                                  updateData['creatorName'] = _creatorController.text.trim();
                                }
                                if (_dateController.text.trim().isNotEmpty) {
                                  try {
                                    final parsedDate = DateFormat('dd.MM.yyyy').parse(_dateController.text.trim());
                                    final originalTimestamp = data['timestamp'];
                                    final originalDate = originalTimestamp is Timestamp
                                        ? originalTimestamp.toDate()
                                        : (originalTimestamp is String
                                            ? DateTime.tryParse(originalTimestamp)
                                            : null);
                                    if (originalDate == null || !isSameDate(parsedDate, originalDate)) {
                                      updateData['timestamp'] = Timestamp.fromDate(parsedDate);
                                    }
                                  } catch (e) {
  ...
  // Helper for refresh/reanalyze action
  Future<void> _reanalyze(BuildContext context, Map<String, dynamic> data) async {
    ...
    final transcriptText = (dataMap['transcript'] as String?)?.trim();
    if (transcriptText != null && transcriptText.isNotEmpty) {
      ...
      if (filePath != null && filePath.isNotEmpty) {
        ...
        showFlushbar('Kein Transkript oder Audio vorhanden.');
        return;
      }
    }
...
class _Section extends StatefulWidget {
  ...
  final Widget? action; // NEW optional trailing action like on Prophetie screen
  const _Section({
    Key? key,
    required this.title,
    required this.child,
...
class _SectionState extends State<_Section> {
  ...
  Widget build(BuildContext context) {
    return Column(
      ...
      children: [
        // Header (flat) matching Prophetie screen
        InkWell(
