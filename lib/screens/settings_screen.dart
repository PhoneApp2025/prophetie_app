...
class SettingsScreen extends StatefulWidget {
  ...
  final String? initialSection; // e.g. 'language'
...
class _SettingsScreenState extends State<SettingsScreen> {
  ...
  String _selectedLanguageCode = 'de'; // 'de' | 'en'
  String get _selectedLanguageLabel =>
      _selectedLanguageCode == 'en' ? 'English' : 'Deutsch';
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (!mounted) return;
      if (widget.initialSection == 'language' &&
          _languageTileKey.currentContext != null) {
        Scrollable.ensureVisible(
          _languageTileKey.currentContext!,
  ...
  Future<void> _loadLanguage() async {
    ...
    try {
      final doc = await FirebaseFirestore.instance
          ...
          .doc(uid)
          .get();
      final stored = (doc.data()?['appLanguage'] as String?)?.toLowerCase();
      if (stored == 'en' || stored == 'de') {
        setState(() => _selectedLanguageCode = stored!);
        return;
      }
      // No stored pref → resolve from system once and persist as concrete 'de'/'en'
      final sys = _resolveSystemLanguage();
      await _saveLanguage(sys, silent: true);
      setState(() => _selectedLanguageCode = sys);
    } catch (_) {
      // Fallback to system
      final sys = _resolveSystemLanguage();
      setState(() => _selectedLanguageCode = sys);
  ...
  String _resolveSystemLanguage() {
    final Locale sys = WidgetsBinding.instance.platformDispatcher.locale;
  ...
  Future<void> _saveLanguage(String code, {bool silent = false}) async {
    if (code != 'de' && code != 'en') code = 'de';
    final uid = FirebaseAuth.instance.currentUser?.uid;
    if (uid == null) return;
    ...
    // Update the global locale notifier so the UI switches immediately
    localeNotifier.value = Locale(code);
    if (!silent) {
      // Zeige die Flushbar erst NACHDEM das BottomSheet sauber geschlossen wurde,
      // um Navigator-Assertions zu vermeiden.
      WidgetsBinding.instance.addPostFrameCallback((_) {
  ...
  Future<bool> _reauthenticateUser() async {
    ...
    try {
      if (providerId == 'password') {
        ...
        await user.reauthenticateWithCredential(credential);
      } else if (providerId == 'apple.com' && Platform.isIOS) {
                    ...
                    trailing: Row(
                      ...
                      children: [
                        FutureBuilder<CustomerInfo>(
                          future: PurchaseService().getCustomerInfo(),
                          builder: (context, snapshot) {
                            ...
                            final entitlement =
                                info.entitlements.active['PHONĒ+'];
                            String planText = 'Kein Plan';
                            if (entitlement != null) {
                              final prodId = entitlement.productIdentifier;
                              if (prodId == 'phone_plus_monthly') {
                                planText = 'Monatlich';
                              } else if (prodId == 'phoneplus_yearly') {
                                planText = 'Jährlich';
                              } else {
                                planText =
                                    'Aktiv'; // Fallback falls Produkt-ID neu/anders ist
                              }
                            }
                            return Text(
                              planText,
