...
class TraeumeScreenState extends State<TraeumeScreen> {
  ...
  Future<void> loadTraeumeFromFirestore() async {
    ...
    // Neue: Traeume mit isAnalyzed == false analysieren (asynchron und mit Fehlerbehandlung)
  ...
  Future<void> analyzeTraum(
    ...
    try {
      if (audioUrl != null && audioUrl.isNotEmpty) {
        // Use central transcription service for remote audio and trigger analysis
        await transcribeAndPrepareAnalysis(
          filePath: audioUrl,
          docId: docId,
          collectionName: 'traeume',
                ...
                children: [
                  ...
                  TextField(
                    ...
                    decoration: InputDecoration(
                      hintText: "Gib hier dein Traum ein...",
                  ...
                  TextField(
                    ...
                    decoration: InputDecoration(
                      hintText: "Von wem stammt der Traum?",
                      hintStyle: TextStyle(color: Theme.of(context).hintColor),
                      border: OutlineInputBorder(
                        borderRadius: BorderRadius.circular(12),
                        borderSide: BorderSide.none,
  ...
  void retryFailedUpload(String traumId) async {
    if (!uploadingTraumIds.contains(traumId)) {
      ...
      if (traum.filePath != null && traum.filePath!.isNotEmpty) {
        try {
          // Versuche erneut den Traum zu speichern
          await saveTraeume();
    ...
    return Slidable(
      ...
      endActionPane: ActionPane(
        ...
        dismissible: DismissiblePane(
          onDismissed: () {
            // Immediately remove from list to avoid Slidable assertion
            setState(() {
        ...
        children: [
          SlidableAction(
            onPressed: (_) async {
              ...
              buffer.writeln('Titel: ${t.title ?? t.text}');
              ...
              final shareText =
                  ...
                  'Träume direkt auf dein Smartphone!';
...
extension TraumCopyWith on Traum {
  Traum copyWith({
    String? id,
    String? text,
    String? label,
...
/// Speichert alle Träume in Firestore (jeweils als einzelnes Dokument)
Future<void> saveTraeume() async {
  ...
  for (var traum in traeume) {
    final id = traum.id ?? const Uuid().v4();
    ...
    await FirebaseFirestore.instance
        ...
        .set({
          ...
          'label': traum.label ?? 'NEU',
          // Optional: Weitere Felder sichern, falls benötigt
          'isFavorit': traum.isFavorit,
          'mainPoints': traum.mainPoints,
          'summary': traum.summary,
          'verses': traum.verses,
          'questions': traum.questions,
          'similar': traum.similar,
          'creatorName': traum.creatorName,
          'title': traum.title,
          'storiesExamplesCitations': traum.storiesExamplesCitations,
          'actionItems': traum.actionItems,
          'relatedTopics': traum.relatedTopics,
          'transcript': traum.transcript,
          'isAnalyzed': traum.isAnalyzed ?? false,
        });
  }
}

// --- TraumDetail Widget ---
class TraumDetail extends StatefulWidget {
  final Traum traum;
  const TraumDetail({super.key, required this.traum});

  @override
  State<TraumDetail> createState() => _TraumDetailState();
}

class _TraumDetailState extends State<TraumDetail> {
  bool isExpanded = false;

  @override
  Widget build(BuildContext context) {
    final text = widget.traum.transcript;
...
class _TranscriptSection extends StatefulWidget {
  final Traum traum;
  const _TranscriptSection({required this.traum});
...
class _TranscriptSectionState extends State<_TranscriptSection> {
  ...
  @override
  Widget build(BuildContext context) {
    final traum = widget.traum;
    final transcript = traum.transcript ?? '';
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        const Text(
