...
class _HomeScreenState extends State<HomeScreen> {
  ...
  void initState() {
    ...
    _connectionsPageController = PageController(
      viewportFraction: 0.86, // zeigt links/rechts angeschnittene Karten
      initialPage: 0,
    );
    _connectionsPageController.addListener(() {
      final page = _connectionsPageController.page ?? 0.0;
      if (page != _currPage && mounted) {
                ...
                SliverToBoxAdapter(
                  child: FutureBuilder<List<ConnectionPair>>(
                    ...
                    builder: (ctx, snap) {
                      ...
                      final double maxByWidth = isTablet ? screenW * 0.68 : screenW * 0.88;
                      final double maxByHeight = size.height * (isTablet ? 0.22 : 0.34);
                      final double cardHeight = math.min(isTablet ? 220 : 300, math.min(maxByWidth, maxByHeight));

                      return Padding(
                        padding: const EdgeInsets.symmetric(horizontal: 8.0),
                        child: SizedBox(
                          height: cardHeight * 1.08, // etwas Luft für Scale
                          child: PageView.builder(
                            ...
                            itemBuilder: (ctx, index) {
                              // Wie weit ist die Karte vom Fokus entfernt?
                              final delta = (_currPage - index).abs();
                              // 1.0 (fokussiert) bis ~0.9 (seitlich)
                              final scale = (1 - (delta * 0.08)).clamp(0.9, 1.0);
                              // 1.0 bis ~0.65
                              final opacity = (1 - (delta * 0.45)).clamp(0.65, 1.0);
                        ...
                        FutureBuilder<List<Widget>>(
                          ...
                          builder: (context, snapshot) {
                            if (snapshot.connectionState ==
                              ...
                              // Höhe nicht fixieren – Karte passt sich Inhalt an
                              return Column(
                                mainAxisSize: MainAxisSize.min,
                                children: [
