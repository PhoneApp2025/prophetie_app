...
class _LoginScreenState extends State<LoginScreen> {
        ...
        actions: [
          ...
          TextButton(
            onPressed: () async {
              ...
              if (email.isEmpty) {
                showFlushbar('Bitte gib eine E-Mail-Adresse ein.');
                return;
              }
              try {
                ...
                if (mounted) {
                  showFlushbar('E-Mail zum Zurücksetzen wurde versendet.');
                }
                Navigator.of(context).pop(); // Dialog schließen
              } catch (e) {
                if (mounted) {
  ...
  void _login() async {
    ...
    try {
      ...
      if (mounted) {
        showFlushbar('Unbekannter Fehler: ${e.toString()}');
      }
    } finally {
      if (mounted) {
        setState(() {
  ...
  Future<void> _signInWithGoogle() async {
    ...
    try {
      ...
      if (mounted) {
        showFlushbar('Google Anmeldung fehlgeschlagen: ${e.toString()}');
      }
    } finally {
      if (mounted) {
        setState(() {
  ...
  Future<void> _signInWithApple() async {
    ...
    try {
      ...
      if (mounted) {
        showFlushbar('Apple Anmeldung fehlgeschlagen: ${e.toString()}');
      }
    } finally {
      if (mounted) {
        setState(() {
  ...
  Widget build(BuildContext context) {
    ...
    final headerTopPosition = _slideUp
        ? screenHeight * 0.075
        : screenHeight * nonSlideUpFraction;
    return Scaffold(
      backgroundColor: Colors.black,
      body: Stack(
        fit: StackFit.expand,
